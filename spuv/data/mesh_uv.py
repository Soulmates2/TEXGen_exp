...
def file_from_uv_dir(uv_dir, mes_name="model.obj", uv_name="model.png", caption_idx=None):
    if caption_idx is not None:
        uv_name = uv_name.replace('.png', f'{caption_idx:02d}.png')
...
class ObjaverseDataset(Dataset):
    ...
    def try_get_item(self, index):
        ...
        caption_idx = uv_data.get("caption_idx", None)
        mesh_file_path, uv_image_path = file_from_uv_dir(uv_dir, self.cfg.mesh_name, self.cfg.uv_name, caption_idx)
        # import ipdb; ipdb.set_trace(context=25)
        ...
        if caption_idx is not None:
            out["caption_idx"] = int(caption_idx)
